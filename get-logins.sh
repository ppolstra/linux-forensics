...
do
  last -aiFwx -f $logfile | \
    ...
    | tee -a /var/lib/mysql-files/logins.csv 
done

echo "who-what;terminal-event;start;stop;elapsedTime;ip" \
   | tee /var/lib/mysql-files/login-fails.csv
...
do
  lastb -aiFwx -f $logfile | \
    awk '{print substr($0, 1, 8) ";" substr($0, 10, 13) ";" \
       ...
       ";" substr($0, 88, 15)}' \
    | sed 's/[[:space:]]*;/;/g' | sed 's/[[:space:]]+\n/\n/' \
    | tee -a /var/lib/mysql-files/login-fails.csv 
done
...
load data infile "/var/lib/mysql-files/logins.csv" ignore
   into table logins
...
load data infile "/var/lib/mysql-files/login-fails.csv" ignore
